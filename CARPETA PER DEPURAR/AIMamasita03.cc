...
//AUXILIAR FUNCTIONS
  ...
  Pos findNearestPlace(const int& unitId) {
    int finalPlace = 0;
    int dist = manhattanDist(unit(unitId).pos, myCities[0].position);
    int size = myCities.size();
    for (int i = 1; i < size; ++i) {
      int auxDist = manhattanDist(unit(unitId).pos, myCities[i].position);
      if (myCities[i].isAvailable and auxDist < dist) {
        dist = auxDist;
        finalPlace = i;
      }
    }
    if (myCities[finalPlace].isAvailable) {

...
//ATACKING
  void checkNearly(const int& unitId) {
    for(int dirInt = BOTTOM; dirInt != NONE; ++dirInt) {
      ...
      bool isUnit = (adjecent.unit_id != -1);
      bool isEnemy =(myUnits.find(adjecent.unit_id) == myUnits.end());
      ...
      if(isUnit) {
        if (isEnemy and !inMyPath) {
          ...
          return;
        } else if (!isEnemy and inMyPath) {
          if(!myUnits[adjecent.unit_id].path.empty()) {
            if(unit(adjecent.unit_id).pos + myUnits[adjecent.unit_id].path.top() == unit(unitId).pos) {
              myUnits[unitId].path.push(static_cast<Dir>((dir + 1) % 4));
              myUnits[unitId].path.push(static_cast<Dir>((dir + 3) % 4));
            }
          }
        }
      }  
